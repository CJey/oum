...
func guide() {
	...
	fmt.Printf("2. install dependency: openvpn ipset iptables openssl sqlite3\n")
	fmt.Printf("\n")
	...
	fmt.Printf("3. start openvpn service with server.conf # service openvpn start\n")
	fmt.Printf("4. oum serv add /etc/openvpn/server.conf # assume interface name is 'oum'\n")
	fmt.Printf("\n")

	fmt.Printf("[Add User]\n")
	fmt.Printf("1. oum add cjey # add a user named <cjey>\n")
	fmt.Printf("2. pass the output url link or the qrcode image to cjey\n")
	fmt.Printf("3. cjey must install mobile app Freeotp(Android) or Google Authenticator(iOS)\n")
	fmt.Printf("4. cjey use Freeotp or Google Authenticator register the qrcode from step 2\n")
	fmt.Printf("\n")
	...
	fmt.Printf("# Enable ip forwarding and nat feature\n")
