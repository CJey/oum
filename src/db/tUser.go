...
func (t tUser) history() map[uint][]string {
	hist := map[uint][]string{
		1: []string{
			...
			"allow_net text not null default '', -- IP白名单，IP支持CIDR格式，多个net/ip用逗号分割(csv)",
			"allow_domain text not null default '', -- 域名白名单，域名会先被解析成IP, 多个域名用逗号分割(csv)",
			"allow_city text not null default '', -- 城市白名单，匹配ipip.net的地址解析结果(ref: 免费API http://www.ipip.net/api.html)，格式为(不要附加行政后缀): 国家名[>省名[>城市名]]，例: 中国，中国>江苏，中国>江苏>南京，中国>上海>上海; 多个城市用逗号分割(csv)",
			"ipset_assign text not null default '', -- ipset名称，多个用逗号分割(csv); 用户设备完成ip配置后，将分配给设备的ip地址追加到所有的set中; set如果已经存在，则应当兼容hash:ip类型，如果不存在，oum将以此名称自动创建类型为hash:ip的set",
			"ipset_access text not null default '', -- 格式和约束参见ipset_access; 用户设备完成ip配置后，将该设备的远程介入ip地址追加到所有的set中",
